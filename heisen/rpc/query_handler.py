...
def get_server():
    server = Server('http://aliehsanmilad:Key1_s!3cr3t@{0}'.format(settings.CORE_SERVICES))
    return server
...
def send_request(*args):
    ...
    try:
        result = get_server()._request(*args)
        ...
        handled_error = handle_errors(e, rpc_id)

        message_error = 'Func: {}, Error Type: {}, Error: {}, Result: {}, Message: {}'.format(
            str(args[0]), type(e), handled_error, result, str(e)
        )
        output['error'] = handled_error, message_error
        ...
        if hasattr(e, 'strerror') and (e.strerror == "No route to host"):
            ...
            msg += ' {0}'.format(settings.CORE_SERVICES)
